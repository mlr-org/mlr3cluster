...
- `clust.SimpleKMeans` now declares `min_density` as a double instead of
  an integer with a minimum of 1, since Weka accepts fractional canopy
  densities.
- `clust.specc` now documents the default of `degree` as 1 instead of 3,
  matching the kernlab kernel functions.
- `clust.xmeans` now allows `S = 0`, which Weka accepts, matching the
  seed bound of `clust.SimpleKMeans`.
